...
namespace CodeGenerator
{
    public class CodeGeneratorCommand : Command
    {
        ...
        {
            Instance = this;
            Panels.RegisterPanel(PlugIn, typeof(CodeGeneratorPanel), "Compas Code Generator", null);
        }
        ...
        {
            var panelId = CodeGeneratorPanel.PanelId;
            Panels.OpenPanel(panelId);
            
            string layer = SelectLayer(doc);
            if (layer != null)
            {
                Rhino.RhinoApp.WriteLine("The selected object's layer is '{0}'", layer);
            }
